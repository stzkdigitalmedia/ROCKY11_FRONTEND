...
const TodayDepositRequests = () => {
  ...
  const fetchData = async (page = 1, startDate, endDate) => {
    ...
    try {
      ...
      const response = await axios.post('https://www.powerdreams.co/api/online/request/getTodayPowerPayDepositUserDetails', payload);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 lg:ml-64">
        <AdminHeader 
          title="Today Deposit Requests"
          subtitle="Users with deposit requests today"
        />
          ...
          <div className="gaming-card p-4 sm:p-6">
            ...
            {loading ? (
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm">No deposit requests for the selected date range</p>
              </div>
            ) : (
              <>
                      ...
                      {filteredData.map((item, index) => (
                        <tr key={index} className="border-b border-gray-100">
                          <td className="py-4 px-4">
                            <p className="text-sm font-medium text-gray-900">{(currentPage - 1) * 50 + index + 1}</p>
                          </td>
                          <td className="py-4 px-4">
                            <p className="text-sm font-medium text-gray-900">{item?.clientUserName}</p>
                          </td>
                          <td className="py-4 px-4">
                            <p className="text-sm text-gray-600">{item?.totalRequests}</p>
                          </td>
                          <td className="py-4 px-4">
                            <p className="text-sm font-medium text-green-600">₹{parseFloat(item?.amount || 0).toLocaleString()}</p>
                          </td>
                        </tr>
                      ))}
                    </tbody>
...
export default TodayDepositRequests;