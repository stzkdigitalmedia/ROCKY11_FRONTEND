...
const UsersList = ({ onUserDeleted, onUsersCountChange, onBalanceSumChange }) => {
  ...
  const [loadingBalanceIds, setLoadingBalanceIds] = useState({});
  ...
  const handleGetBalance = async (account) => {
    ...
    setLoadingBalanceIds(prev => ({ ...prev, [id]: true }));
    try {
      await createBalanceLog(id);
      const balance = await fetchSubUserBalance(id);
      setSubAccountBalances(prev => ({ ...prev, [id]: balance }));
    } catch (error) {
      toast.error('Failed to fetch balance');
    } finally {
      setLoadingBalanceIds(prev => ({ ...prev, [id]: false }));
    }
  };
  ...
  const openBonusModal = (user) => {
    setBonusUser(user);
    setBonusAmount('');
    setBonusRemark('add manual bonus');
  ...
  const addManualBonus = async () => {
    ...
    try {
      ...
      await apiHelper.post('/bonus/add-manual-bonus', payload);
            ...
            {subAccountsLoading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {userSubAccounts.filter(account => account?.status !== 'Reject').map((account, index) => (
                      <tr key={account?.id || account?._id || index} className="border-b border-gray-100">
                        ...
                        <td className="py-4 px-4">
                          <p className="text-sm font-semibold text-green-600">
                            ₹ {(subAccountBalances[account?.id || account?._id] || 'Pending').toLocaleString()}
                          </p>
                        </td>
                        <td className="py-4 px-4">
                        ...
                        <td className="py-4 px-4">
                          <button
                            ...
                            disabled={loadingBalanceIds[account?.id || account?._id]}
                            className="px-3 py-1 bg-blue-600 text-white text-xs rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1"
                          >
                            {loadingBalanceIds[account?.id || account?._id] ? (
                              <><span className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin"></span> Loading...</>
                            ) : 'Get Bal'}
                          </button>
                        </td>
                      </tr>
      ...
      {showBonusModal && (
        <div className="fixed inset-0 modal-overlay flex items-center justify-center p-4 z-50">
          <div className="gaming-card p-4 sm:p-6 max-w-md w-full mx-4">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Add Manual Bonus</h2>
                <p className="text-gray-600 text-sm mt-1">Add bonus for {bonusUser?.clientName}</p>
              </div>
              <button onClick={closeBonusModal} className="text-gray-400 hover:text-gray-600">
                <X size={20} />
              </button>
            </div>
