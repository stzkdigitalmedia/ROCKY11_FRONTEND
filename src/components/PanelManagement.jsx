...
const PanelManagement = () => {
  ...
  const fetchPanelSetting = async () => {
    try {
      const response = await apiHelper.get("/panelSetting/getPanelSetting");
      const data = response?.data;
      setPanelSetting(data);
      setSettingForm({
        noOfID: data?.noOfID ?? 1,
        isRoundRobin: data?.isRoundRobin ?? false,
      });
    } catch (error) {
      showToast(error?.message || "Failed to fetch panel setting", "error");
    }
  };

  const updatePanelSetting = async (e) => {
    e.preventDefault();
    setSettingLoading(true);
    try {
      await apiHelper.patch(
        `/panelSetting/updatePanelSetting/${panelSetting._id}`,
        settingForm,
      );
      showToast("Panel setting updated successfully", "success");
      fetchPanelSetting();
    } catch (error) {
      showToast(error?.message || "Failed to update panel setting", "error");
    } finally {
  ...
  const fetchGames = async () => {
    ...
    try {
      const response = await apiHelper.get(
        "/game/getAllGames_without_pagination",
      );
  ...
  const fetchPanels = async (page = 1) => {
    ...
    try {
      const response = await apiHelper.get(
        `/panel/getAllPanels?page=${page}&limit=100`,
      );
  ...
  const handleDefaultToggle = async (panelId, currentDefault, panel) => {
    setPanels((prevPanels) =>
      prevPanels.map((p) =>
        (p._id || p.id) === panelId ? { ...p, isDefault: !currentDefault } : p,
      ),
    );
    ...
    try {
      ...
      showToast(
        error.message || "Failed to update panel default status",
        "error",
      );
    }
  };
  ...
  const handleShowToggle = async (panelId, currentShow, panel) => {
    setPanels((prevPanels) =>
      prevPanels.map((p) =>
        (p._id || p.id) === panelId ? { ...p, isShow: !currentShow } : p,
      ),
    );
  ...
  const handleUpdatePanel = async (e) => {
    ...
    try {
      ...
      await apiHelper.patch(
        `/panel/updatePanel_in_all_tables/${panelId}`,
        editFormData,
      );
  ...
  return (
    <div>
      {/* Panel Setting Card */}
      <div className="gaming-card p-4 sm:p-6 mb-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">
          Panel Settings
        </h2>
        {panelSetting ? (
          <form
            onSubmit={updatePanelSetting}
          ...
          >
            <div>
              ...
              <input
                type="number"
                min={1}
                value={settingForm.noOfID}
                onChange={(e) =>
                  setSettingForm((prev) => ({
                    ...prev,
                    noOfID: Number(e.target.value),
                  }))
                }
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  checked={settingForm.isRoundRobin}
                  onChange={(e) =>
                    setSettingForm((prev) => ({
                      ...prev,
                      isRoundRobin: e.target.checked,
                    }))
                  }
            ...
            <button
              ...
              className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 mb-2"
            >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <div>
            ...
            <p className="text-gray-600 text-sm mt-1">
              Create and manage exchange panels
            </p>
          </div>
          <button
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {panelGroup.length} panel
                          {panelGroup.length > 1 ? "s" : ""}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="text-sm font-bold">
                        <span className="text-gray-700">GroupID: </span>
                        <span className="text-green-600">
                          {panelGroup[0]?.groupId || 0}
                        </span>
                      </div>
                            ...
                            <div className="flex items-center gap-2">
                              <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center">
                                <span className="text-xs font-medium text-blue-600">
                                  {(panel.userName || panel.username || "U")
                                    .charAt(0)
                                    .toUpperCase()}
                                </span>
                              </div>
                              <span className="text-sm font-medium text-gray-900">
                                {panel.userName || panel.username || "N/A"}
                              </span>
                            </div>
                          </td>
                          <td className="py-3 px-4">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, adminLink: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
