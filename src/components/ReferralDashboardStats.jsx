...
import ReferralUsersList from './ReferralUsersList';
import AddUserForm from './AddUserForm';
import { Users, Plus, Calendar, RotateCcw, ChevronDown } from 'lucide-react';
...
const ReferralDashboardStats = ({ selectedReferralCode }) => {
  ...
  const [selectedBranch, setSelectedBranch] = useState(selectedReferralCode || '');
  ...
  const handleBranchSelect = (branch) => {
    ...
    // Refetch data with new branch
    fetchDashboardSummary(dateRange[0].startDate, dateRange[0].endDate, branch);
  };

  const fetchDashboardSummary = async (startDate, endDate, referalCode = selectedReferralCode || selectedBranch) => {
    ...
    //   const localResponse = await apiHelper.get(`/transaction/dash-summary?startDate=${startUTC}&endDate=${endUTC}`);
      const localResponse = await apiHelper.get(`/referal/getReferal_Wise_DashSummary?startDate=${startUTC}&endDate=${endUTC}&referalCode=${referalCode}`);
    ...
    //   const externalResponse = await apiHelper.get(`/transaction/dash-summary?startDate=${startUTC}&endDate=${endUTC}`);
      const externalResponse = await apiHelper.get(`/referal/getReferal_Wise_DashSummary?startDate=${startUTC}&endDate=${endUTC}&referalCode=${referalCode}`);
      ...
      const todayDepositResponse = await apiHelper.get(`/referal/getDeposit_Users_Tranx_forReferal_Dashboard?startDate=${startUTC}&endDate=${endUTC}&referalCode=${referalCode}`);
      setTodayDepositCount(todayDepositResponse?.userCount || 0);

      // Fetch today withdrawal requests count
      const todayWithdrawalResponse = await apiHelper.get(`/referal/getWithdraw_Users_Tranx_forReferal_Dashboard?startDate=${startUTC}&endDate=${endUTC}&referalCode=${referalCode}`);
  ...
  const handleStatusClick = (status) => {
    navigate(`/referral-status-details?status=${status}`);
  };

  useEffect(() => {
    setSelectedBranch(selectedReferralCode || '');
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
        <>
          <div
            ...
            onClick={() => navigate('/referral-deposit-transactions')}
          >
          ...
          <div
            className="gaming-card p-4 cursor-pointer hover:shadow-lg transition-shadow"
            onClick={() => navigate('/referral-withdrawal-transactions')}
          >
          ...
          <div
            className="gaming-card p-4 cursor-pointer hover:shadow-lg transition-shadow"
            onClick={() => navigate('/referral-user-registrations')}
          >
          ...
          <div
            className="gaming-card p-4 cursor-pointer hover:shadow-lg transition-shadow"
            onClick={() => navigate('/referral-ftd-complete-users')}
          >
          ...
          <div
            className="gaming-card p-4 cursor-pointer hover:shadow-lg transition-shadow"
            onClick={() => navigate('/referral-no-transaction-users')}
          >
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <>
            ...
            <div className="gaming-card p-4 pb-4 cursor-pointer hover:shadow-lg transition-shadow" onClick={() => navigate('/referral-deposit-transactions')}>
            ...
            <div className="gaming-card p-4 pb-4 cursor-pointer hover:shadow-lg transition-shadow" onClick={() => navigate('/referral-withdrawal-transactions')}>
      ...
      <div className="gaming-card p-4 sm:p-6">
        ...
        <ReferralUsersList
          ...
          selectedReferralCode={selectedReferralCode || selectedBranch}
        />
      </div>
