...
const ReferralUsersList = ({ onUserDeleted, onUsersCountChange, onBalanceSumChange, selectedReferralCode }) => {
  ...
  const downloadUsersExcel = async () => {
    try {
      const referralCode = selectedReferralCode || '';
      const response = await apiHelper.get(`/referal/getAllReferal_BranchUsers?referalCode=${referralCode}&page=1&limit=100000`);
  ...
  const fetchUsers = async (currentPage = 1) => {
    ...
    if (!referralCode) {
      ...
      setTotalUsers(0);
      setBalanceSum(0);
      onUsersCountChange?.(0);
      onBalanceSumChange?.(0);
      return;
    }
    ...
    try {
      ...
      const response = await apiHelper.get(
        `/referal/getAllReferal_BranchUsers?${params.toString()}`
      );
